...
@implementation CoreDataPart2AppDelegate
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	//riempiamo l'interfaccia con i valori del campo selezionato
	nameField.text = [[contattiList objectAtIndex:indexPath.row] nome];
	surnameField.text = [[contattiList objectAtIndex:indexPath.row] cognome];
	nickField.text = [[contattiList objectAtIndex:indexPath.row] nick];
	
	//aggiorniamo currentRecord
	currentRecord = indexPath.row;
	
	//aggiorniamo il testo del pulsante 
	[addButton setTitle:@"Modifica" forState:UIControlStateNormal];
}
...
- (NSManagedObjectModel *)managedObjectModel {
    ...
    NSString *modelPath = [[NSBundle mainBundle] pathForResource:@"CoreDataPart2" ofType:@"momd"];
